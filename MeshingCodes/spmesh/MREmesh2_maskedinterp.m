...
%   July 2009
%ooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo
function MREmesh2_maskedinterp(fname)
...
if (todo == 2 || todo == 3)
    ...
    ! mkdir -p MRPE
    ! mkdir -p MRPE/INV
    ! mv *.ci pressure.s1c 3D_MRPE* MRPE-* MRPE/
...
% Old Method use values from outside the mask, which causes boundary errors.
% % real displacements
% rdisp(:,1)=nod(:,1);
% rdisp(:,2)=interp3(xx,yy,zz,pDr(:,:,:,1),nod(:,2),nod(:,3),nod(:,4),'spline');
% rdisp(:,3)=interp3(xx,yy,zz,pDr(:,:,:,2),nod(:,2),nod(:,3),nod(:,4),'spline');
% rdisp(:,4)=interp3(xx,yy,zz,pDr(:,:,:,3),nod(:,2),nod(:,3),nod(:,4),'spline');
% 
% % imaginary displacements
% cdisp(:,1)=nod(:,1);
% cdisp(:,2)=interp3(xx,yy,zz,pDi(:,:,:,1),nod(:,2),nod(:,3),nod(:,4),'spline');
% cdisp(:,3)=interp3(xx,yy,zz,pDi(:,:,:,2),nod(:,2),nod(:,3),nod(:,4),'spline');
% cdisp(:,4)=interp3(xx,yy,zz,pDi(:,:,:,3),nod(:,2),nod(:,3),nod(:,4),'spline');

% New Method uses a masked interpolation which does not use NaN values
% real displacements
xv=dx:dx:(nx)*dx;
yv=dy:dy:(ny)*dy;
zv=0:dz:(nslice+1)*dz;
maskjunk=zeros(size(pDr,1),size(pDr,2),size(pDr,3));
maskjunk(:,:,2:end-1)=mask;

rdisp(:,1)=nod(:,1);
junk=pDr(:,:,:,1);junk(~maskjunk)=NaN;
disp('Interpolating')
rdisp(:,2)=SplineInterp3D_withnans_unstructured(xv,yv,zv,junk,nod(:,3),nod(:,2),nod(:,4),'spline');
disp('Interp1')
junk=pDr(:,:,:,2);junk(~maskjunk)=NaN;
rdisp(:,3)=SplineInterp3D_withnans_unstructured(xv,yv,zv,junk,nod(:,3),nod(:,2),nod(:,4),'spline');
disp('Interp2')
junk=pDr(:,:,:,3);junk(~maskjunk)=NaN;
rdisp(:,4)=SplineInterp3D_withnans_unstructured(xv,yv,zv,junk,nod(:,3),nod(:,2),nod(:,4),'spline');
disp('Interp3')
% imaginary displacements
cdisp(:,1)=nod(:,1);
junk=pDi(:,:,:,1);junk(~maskjunk)=NaN;
cdisp(:,2)=SplineInterp3D_withnans_unstructured(xv,yv,zv,junk,nod(:,3),nod(:,2),nod(:,4),'spline');
disp('Interp4')
junk=pDi(:,:,:,2);junk(~maskjunk)=NaN;
cdisp(:,3)=SplineInterp3D_withnans_unstructured(xv,yv,zv,junk,nod(:,3),nod(:,2),nod(:,4),'spline');
disp('Interp5')
junk=pDi(:,:,:,3);junk(~maskjunk)=NaN;
cdisp(:,4)=SplineInterp3D_withnans_unstructured(xv,yv,zv,junk,nod(:,3),nod(:,2),nod(:,4),'spline');
disp('Interp6')

%% Transform Nodal Coordinates according with DirIndex directions
nod_transform(nod,rdisp,cdisp,DirIndex,name);
end
