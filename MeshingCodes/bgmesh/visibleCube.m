function [hiso,hcap,pb]=visibleCube(stack,stackInfo,isoV,Nxyz,argin)
...
% If we want to send the figure back to labview to display it, we can use
% another matlab instance to do the compuation and save the figure
% ('visible' is 'off') to jpg file.
%
...
%% scale input 
[rows,cols,slices]=size(stack);
D = zeros(floor([ rows/Nxyz(1), cols/Nxyz(2), slices/Nxyz(3)]));
for i = 1 : floor(slices/Nxyz(3))
	D(:,:,i) = stack( 1:Nxyz(1):end,1:Nxyz(2):end, i*Nxyz(3) ) ;
end
