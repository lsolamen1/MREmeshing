%==========================================================================
% bgmesh.m [brain group mesh]
% DESCRIPTION
% Meshing Written by: Hamid Ghadyani, PhD
%
%
% Meshing Code Adapted for MRE by: 
%   Ligin Solamen, PhD Candidate
%   Thayer School of Engineering
%   Dartmouth College
%   August 2015
%
% .dsp and .v3r adapted from MREmesh2.m
%
%==========================================================================

function bgmesh(fname)
...
%SpacingBetweenSlices=parms.tags(1,24);
...
if min(nodtemp(:,3))<0
    nodtemp2(:,3)=nodtemp(:,3)*1e-3+min(nodtemp(:,3))*1e-3+(2/3)*DirIndex(4,3)*1e-3;
else
nodtemp2(:,3)=nodtemp(:,3)*1e-3+(2/3)*DirIndex(4,3)*1e-3;
end
...
spregions=input('Do you want to define a subregion for soft priors?: Yes=1, Default=No>>'); 
if spregions==1
    spfile=sprintf(input('Name of Mask defining homogenous region:>>'));
    clear mask;
    spfile=char(spfile);

    softprior(base,1,0,0,spfile)
    load regs.mat
...
genrunfilev8poro(base,itmax,spregions);
...
new_disp(:,1) = displ(:,1);
new_disp(:,3) = displ(:,Index_1(2)-3+1)*DirIndex(1,Index_1(2));
new_disp(:,2) = displ(:,Index_2(2)-3+1)*DirIndex(2,Index_2(2));
new_disp(:,4) = displ(:,Index_3(2)-3+1)*DirIndex(3,Index_3(2));
...
new_cdisp(:,1) = cdispl(:,1);
new_cdisp(:,3) = cdispl(:,Index_1(2)-3+1)*DirIndex(1,Index_1(2));
new_cdisp(:,2) = cdispl(:,Index_2(2)-3+1)*DirIndex(2,Index_2(2));
new_cdisp(:,4) = cdispl(:,Index_3(2)-3+1)*DirIndex(3,Index_3(2));
